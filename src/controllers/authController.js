...
export const updateAdminPriviledge = async (req, res) => {
  try {
    const { adminId } = req.params;
    const { priviledges } = req.body;
    ...
    res.status(200).json({ message: "Admin privileges updated successfully", updatedAdmin });
  } catch (err) {
    res.status(500).json({ message: "Error updating admin privileges", err });
  }
};
